...
class Assembly_FASTA_new(Assembly_FASTA):
	...
	def calculate_coverage_values_chromosomes(self, bin_size, feature_1, feature_1_label, feature_2, feature_2_label, mode_numbers):
		...
		#create data structure for two features
		for c in self.chromosomes_windows_coverage_numbers_feat_2:
			for pos in self.chromosomes_windows_coverage_numbers_feat_2[c]:
				self.chromosomes_windows_coverage_numbers_feat_1[c][pos]=[self.chromosomes_windows_coverage_numbers_feat_1[c][pos], self.chromosomes_windows_coverage_numbers_feat_2[c][pos]]

###############################################################################################################################################################################
class Plot(Plot):

	def plot_features_chromosomes(self, bin_size, features_chroms, max_len, label_1, label_2, species, no_fill, mode_numbers):

		plt.rcParams["font.family"]= "Arial"

		fig, axs = plt.subplots(nrows=len(features_chroms), ncols=1, figsize=(14,10), sharey=True, sharex=True, tight_layout=True)
		title = fig.suptitle("Content of features along chromosomes "+label_1+" and "+label_2+" - "+species, fontsize=10)
...
def main():
	...
	parser=argparse.ArgumentParser(description="Plot content of two features (e.g., genes vs repeats) along the chromosomes using CHROM files of genome annotations")
	...
	print("Extracting "+args.feature_1_name+" and "+args.feature_2_name+" content for windows of "+str(args.bin_size)+" bp ...")
	my_assembly.calculate_coverage_values_chromosomes(bin_size=int(args.bin_size), feature_1=my_feature_1.chrom_features, \
													  feature_1_label=args.feature_1_name, feature_2=my_feature_2.chrom_features, \
													  feature_2_label=args.feature_2_name, mode_numbers=args.numbers)

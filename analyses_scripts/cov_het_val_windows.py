...
class BAM_alignment():
	...
	def calculate_average_coverages_snps_chromosomes(self, bin_size, snps, uncalled):
		...
		my_het_windows="het_windows.txt"
		fh_out=open(my_het_windows, "w")
		for c in self.chromosomes_windows_depths_snps:
			for w in self.chromosomes_windows_depths_snps[c]:
				fh_out.write(str(self.chromosomes_windows_depths_snps[c][w][1])+"\n")

...
class Plot():
	...
	def plot_coverage_heterozygosity_chromosomes(self, snps_and_depths, average_depth, max_het, species, chrom_lengths, no_fill):

		with open("het_snps.txt", "w") as fh_out:

			for i in snps_and_depths.keys():
				y_snps = []

				for p, value in sorted(snps_and_depths[i].items()):
					fh_out.write(species+"/t"+str(value[1])+"\n")
...
def main():
	...
	parser.add_argument("max_het", help="Max. heterozygosity (percent SNPs) to plot on y axis (for coverage normalization)")
	...
	my_plot.plot_coverage_heterozygosity_chromosomes(snps_and_depths=my_alignment.chromosomes_windows_depths_snps, average_depth= my_alignment.average_depth, max_het=int(args.max_het), species=args.species, chrom_lengths=my_alignment.chromosome_lengths, no_fill=args.no_fill)
	print("All done!\n"+200*"-"+"\n")

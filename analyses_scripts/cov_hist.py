...
# Generates coverage depth histogram plot (per base coverage depth histogram) from an input BAM file.
# It requires that SAMtools is in the path. If depths.txt file is already in the current directory this is used by default.
...
class Alignment():
	...
	def plot_histogram_coverage(self, out_plot, no_bins, max_depth):
		...
		title = plt.title('Coverage depth histogram')
...
def main():
	...
	parser.add_argument("out_plot", help="Output file with appropriate extension to save the plot (*.svg or *.png)")
	...
	#read BAM file and calculate per base coverages
	...
	print(f"\n## Average per base coverage depth: {np.mean(my_alignment.depths_df['depth']):.2f} ##\n")
	print("All done!")

	...
	def plot_snps_chromosomes(self, snps_chroms, max_len, species, bin_size, no_fill):
		...
		if len(snps_chroms.keys())>1:
			...
			for i in snps_chroms.keys():
				...
				l1 = axs[no].plot(position, y_feat1_coverage, color="firebrick", label="no. of SNPs")
				if not no_fill:
...
def main():
	...
	print("\n###############################################################################################################################################")
	print("Estimating number of SNPs (homozygous + heterozygous) for chromosome windows and plotting them along the chromosomes of the reference genome ...")
	print("##################################################################################################################################################\n")

	parser=argparse.ArgumentParser(description="Plot SNP numbers along chromosomes of the reference genome")
	...
	parser.add_argument("bin_size", help="Size of bins in bp (SNPs' number plotted for each bin)")
	...
	parser.usage = 'python3 chromgenomeplot.py snps_number [positional arguments]'

...
###########################################################################################################################################################################################################################
# Plots GC content and feature content along chromosomes using bed file of annotations (each bed annotation must contain sorted annotations by genome position annotations)
##############################################################################################################################################################################################################
		...
		for chromosome in self.scaffolds_seqs.keys():
			...
			for nuc in self.scaffolds_seqs[chromosome]:
				...
				if (position+1)%bin_size==0:
					...
					self.chromosomes_windows[chromosome][start_window] = (feature_1_coverage, (count_GC/(count_GC+count_AT))*100)
					#Add depth , snp percent  of end position
					self.chromosomes_windows[chromosome][position] = (feature_1_coverage, (count_GC/(count_GC+count_AT))*100)
...
class Plot(Plot):
	...
	def plot_features_chromosomes(self, features_chroms, max_len, label_1, species, no_fill):
		...
		title = fig.suptitle('GC and feature content along chromosomes'+" - "+species, fontsize=10)
...
def main():
	...
	parser=argparse.ArgumentParser(description="Plot GC and feature content (e.g., genes or repeats) along the chromosomes.")
	...
	print("Extracting average percent feature 1 and GC content for windows of "+str(args.bin_size)+" bp ...")
